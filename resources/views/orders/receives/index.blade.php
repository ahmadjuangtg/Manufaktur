...
<div id="receiveModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="bg-[#1e293b] border border-white/10 w-full max-w-4xl rounded-2xl shadow-2xl">
        ...
        <form id="receiveForm" method="POST" class="p-10 space-y-8">
            @csrf
            <div class="space-y-3 mb-8">
                <label class="block text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Simpan di Gudang*</label>
                <select name="warehouse_id" class="w-full bg-[#0f172a] border border-white/10 rounded-xl py-4 px-6 focus:border-indigo-500 outline-none text-white text-sm font-bold shadow-inner" required>
            ...
            <div id="additionalItems" class="pt-8 border-t border-white/5 space-y-6">
                <h4 class="text-xs font-bold text-indigo-400 uppercase tracking-widest italic">Tambah Item Tambahan</h4>
                <div id="extraItemList" class="space-y-4"></div>
                <button type="button" onclick="addExtraItem()" class="text-xs font-bold text-indigo-400 hover:text-indigo-300 transition-colors flex items-center gap-2">
                    <i data-lucide="plus-circle" class="w-4 h-4"></i> Tambah Item
                </button>
            </div>
            <div class="flex justify-end gap-6 pt-8 border-t border-white/5">
                <button type="button" onclick="closeReceiveModal()" class="text-sm font-black text-slate-500 uppercase tracking-widest hover:text-white transition-colors">Batal</button>
                <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-10 py-3 rounded-xl font-black text-sm uppercase tracking-widest shadow-lg shadow-emerald-500/20 transition-all">Simpan Penerimaan</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function addExtraItem() {
        ...
        const div = document.createElement('div');
        div.className = 'grid grid-cols-12 gap-4 items-center extra-row';
        ...
        div.innerHTML = `
            ...
            <div class="col-span-1">
                <button type="button" onclick="this.closest('.extra-row').remove()" class="text-rose-500 hover:text-rose-400"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
            </div>
        `;
        list.appendChild(div);
        extraIndex++;
