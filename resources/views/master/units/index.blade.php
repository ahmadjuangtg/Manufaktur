            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="hover:bg-white/5 transition-colors group">
                    ...
                    <td class="px-8 py-4 text-right flex justify-end gap-2">
                        ...
                        <form action="{{ route('units.delete', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus satuan ini?')">
                            @csrf
...
<div id="modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="bg-[#1e293b] border border-white/10 w-full max-w-md rounded-2xl shadow-2xl p-8">
        ...
        <form id="unitForm" action="{{ route('units.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
            ...
            <div class="flex justify-end gap-3 mt-8">
                ...
                <button type="submit" class="bg-indigo-600 text-white px-8 py-2.5 rounded-lg font-bold shadow-lg shadow-indigo-500/20">Simpan Satuan</button>
            </div>
        </form>
    </div>
</div>
