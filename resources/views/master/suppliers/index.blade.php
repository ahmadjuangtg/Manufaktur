@extends('layouts.app', ['title' => 'Master Supplier'])
...
<div id="modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="bg-[#1e293b] border border-white/10 w-full max-w-2xl rounded-2xl flex flex-col max-h-[90vh] overflow-hidden shadow-2xl">
        <div class="p-6 border-b border-white/5 flex justify-between items-center bg-slate-800/50">
            <h3 id="modalTitle" class="text-lg font-bold text-white">Input Supplier</h3>
            ...
            <form id="suppForm" action="{{ route('suppliers.store') }}" method="POST" class="space-y-6" onsubmit="return handleFormSubmit(this)">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label class="block text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-2">Asal Supplier</label>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-1">Nama Supplier*</label>
                ...
                <div class="pt-6 border-t border-white/5">
                    <label class="block text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-1">Mensupply Barang (Master Item)</label>
                    <div class="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto p-4 bg-slate-900/50 rounded-xl border border-white/5 modal-scroll">
                        @foreach($items as $it)
                        <label class="flex items-center gap-2 cursor-pointer group">
                            <input type="checkbox" name="item_ids[]" value="{{ $it->id }}" class="item-checkbox rounded border-white/10 bg-slate-800 text-indigo-600 focus:ring-indigo-500">
                            <span class="text-sm text-slate-400 group-hover:text-white transition-colors">{{ $it->name }}</span>
                        </label>
                        @endforeach
                    </div>
                </div>
