...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div class="flex items-center gap-3 w-full md:w-auto">
            <form action="{{ route('types.index') }}" method="GET" class="relative flex-1 md:w-80">
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="hover:bg-white/5 transition-colors group">
                    ...
                    <td class="px-8 py-4 text-right flex justify-end gap-2">
                        ...
                        <form id="delete-form-{{ $item->id }}" action="{{ route('types.delete', $item->id) }}" method="POST" class="inline">
                            @csrf
                            <button type="button" onclick="confirmAction('Yakin ingin menghapus tipe ini?', () => document.getElementById('delete-form-{{ $item->id }}').submit())" class="p-2 text-slate-500 hover:text-rose-500"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                        </form>
                    </td>
                </tr>
                @empty
...
<div id="modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="bg-[#1e293b] border border-white/10 w-full max-w-md rounded-2xl shadow-2xl p-8">
        ...
        <form id="typeForm" action="{{ route('types.store') }}" method="POST" class="space-y-4" onsubmit="return handleFormSubmit(this)">
            @csrf
            <div>
            ...
            <div class="flex justify-end gap-3 mt-8">
                ...
                <button type="submit" id="submitBtn" class="bg-indigo-600 text-white px-8 py-2.5 rounded-lg font-bold shadow-lg shadow-indigo-500/20 flex items-center gap-2">
                    <span id="btnText">Simpan Tipe</span>
